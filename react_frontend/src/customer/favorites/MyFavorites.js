...
export default function MyFavorites() {
  ...
  useEffect(() => {
    axios.post(`${localhost}/customer/myfavorites`, { 'user_id': userID }, { headers: { 'Authorization': `Bearer ${token}` } })
      .then((res) => {
        setProduct(res.data);
        setIsLoading(false);
      })
  ...
  return (
    <section className="bg-cream-100 font-poppins">
      <Navbar />
      <ScrollToTop />
      <div className="min-w-screen bg-cream-600 px-4 mx-auto lg:px-12 w-full">
          ...
          <div className="col-span-2 relative w-full mb-2 md:mb-0">
            <form className="flex items-center">
              ...
              <div className="relative w-full">
                ...
                <input type="text" id="simple-search" value={search} onChange={(e) => setSearch(e.target.value)} className="block w-full p-1.5 pl-10 text-base text-gray-900 rounded-full border-none focus:outline-none bg-cream-100 outline-none" placeholder="Search" required />
              </div>
            </form>
          </div>
